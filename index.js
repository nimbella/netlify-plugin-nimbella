...
module.exports = {
  ...
  onPostBuild: async ({constants, utils}) => {
    try {
      ...
      for (const redirect of redirects) {
        if (redirect.status === 200) {
          if (redirect.to.startsWith('/.netlify/functions/')) {
            ...
            redirectRules.push(
              `${redirect.from} /.netlify/nimbella-staging/default/${redirectPath} 200!`
            );
          }
        }
      }
      ...
      if (isProject) {
        redirectRules.push(`${redirectPath}* /.netlify/nimbella-staging/:splat 200!`);
      }

      if (isActions && !isProject) {
        redirectRules.push(
          `${redirectPath}* /.netlify/nimbella-staging/default/:splat 200!`
        );
      }
